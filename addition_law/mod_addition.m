...
function doubling (D1, f0, f1, f2, h0, h1, h2, h3)
	...
	//R<x> := PolynomialRing(Parent(Um0));
	
	//print Um0;
	//print Um1;
	//u := Um0 + Um1*x + Um2*x^2 + x^3;
	//c := ty0 - sy2_0 - h0 + (ty1 - sy2_1 - h1)*x + (ty2 - sy2_2 - h2)*x^2 + (ty3 - h3)*x^3;
	//print ExtendedGreatestCommonDivisor(u, c);
	//print "C: ", c;
	print "Alphas:", alpha10, alpha11, alpha12;
	//print res2;
...
doubling2 := function (D1, f0, f1, f2, h0, h1, h2, h3)
	...
	R<x,y> := PolynomialRing(ff_q, 2);
	f := f0 + f1*x + f2*x^2 + x^4;
	E := y^2 + (sy1*x + sy0)*y + (ty3*x^3 + ty2*x^2 + ty1*x + ty0);
	u := D1[1] + D1[2]*x + D1[3]*x^2 + x^3;
	res := ExactQuotient(Resultant(E, y^3 - f, y),u^2);
	//error res;
	res := res/LeadingCoefficient(res);
		
	return [Coefficient(res, x, 0), Coefficient(res, x, 1), Coefficient(res, x, 2), D1[4], D1[5], D1[6]];
end function;


doubling3 := function (D1, f0, f1, f2, h0, h1, h2, h3)
	u10 := D1[1];
	u11 := D1[2];
	u12 := D1[3];
	...
	u := Um0 + Um1*x + Um2*x^2 + x^3;
	c := ty0 - sy2_0 - h0 + (ty1 - sy2_1 - h1)*x + (ty2 - sy2_2 - h2)*x^2 + (ty3 - h3)*x^3;
	c, a, b := ExtendedGreatestCommonDivisor(u, c);
	alpha10 := Coefficient(b, 0);
	alpha11 := Coefficient(b, 1);
	alpha12 := Coefficient(b, 2);
	res2 := ff_q ! c;
	//print u;
	//print "C: ", c;
	//print "Alphas:", alpha10, alpha11, alpha12;
