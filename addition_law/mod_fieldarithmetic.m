...
function modular_inverse_3n_2 (u0, u1, u2, v0, v1, v2)
	...
	if s0 eq 0 and s1 eq 0 and s2 eq 0 then
		R<x> := PolynomialRing(Parent(u0));
	
		//print Um0;
		//print Um1;
		u := u0 + u1*x + u2*x^2 + x^3;
		v := v0 + v2*x + v2*x^2;
		d, t, s := ExtendedGreatestCommonDivisor(u, v);
		return Coefficient(s,0), Coefficient(s,1), Coefficient(s,2), Coefficient(d,0);
	end if;
...
function modular_inverse_3n_4 (u0, u1, u2, c0, c1, c2, c3, c4)
	...
	if s0 eq 0 and s1 eq 0 and s2 eq 0 then
		...
		d, t, s := ExtendedGreatestCommonDivisor(u, c);
		return Coefficient(s,0), Coefficient(s,1), Coefficient(s,2), Coefficient(d,0);
	end if;
